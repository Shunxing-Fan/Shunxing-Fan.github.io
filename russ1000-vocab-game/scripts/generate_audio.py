#!/usr/bin/env python3
"""Generate Google Cloud Text-to-Speech MP3 assets for the Russian vocabulary game."""
...
def build_manifest_items(vocabulary: dict) -> list[dict]:
    items = []
    for entry in vocabulary["items"]:
        item_id = entry["id"]
        text = " ".join(entry["russian"].split())
        items.append(
            {
                "id": item_id,
                "text": text,
...
def write_audio(items: list[dict], token: str, force: bool) -> tuple[int, int]:
...
def main() -> int:
    ...
    items = build_manifest_items(vocabulary)
    token = get_access_token(credentials_path)
    generated, skipped = write_audio(items, token, args.force)
